...
class Issues extends Admin_Controller
{
    ...
    public function view()
    {
        ...
        if ($this->input->post('btn_submit')) {
            $addedtext = "\r\n Posted by: " . $this->session->userdata('user_name');
            $data = array(
                'title' => $this->input->post('title_text'),
                'body' => $this->input->post('body_text') . $addedtext,
            );
    ...
    function CallAPI($method, $url, $data = false)
{
    ...
    curl_setopt($curl, CURLOPT_USERPWD, "atsolutions:" . $this->mdl_settings->setting('git_api_key'));
