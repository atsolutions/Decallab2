...
class Mdl_Quotes extends Response_Model
{
    ...
    public function approve_quote_by_key($quote_url_key, $quote_id)
    {
    ...
    public function reject_quote_by_key($quote_url_key, $quote_id)
    {
        $this->db->where_in('quote_status_id', array(2, 3));
        $this->db->where('quote_url_key', $quote_url_key);
...
public function mark_printed($quote_id)
    ...
    public function mark_sent($quote_id, $to, $client_phone)
    {
        ...
        if ($quote->num_rows()) {
            if ($quote->row()->quote_status_id == 1 or $quote->row()->quote_status_id == 5) {
                $this->db->where('quote_id', $quote_id);
                $this->db->set('quote_status_id', 7);
                $this->db->update('ip_quotes');
            }elseif ($quote->row()->quote_status_id == 7) {
                $this->db->where('quote_id', $quote_id);
                $this->db->set('quote_status_id', 2);
                $this->db->update('ip_quotes');
				$message = urlencode("Your design is ready. Please check your email. Team Decallab.");
				$homepage = file_get_contents('http://sms1.cardboardfish.com:9001/HTTPSMS?S=H&UN=janisklavins1&P=juras1022&DA=' . $client_phone . '&SA=DECALLAB&M=' . $message);
			}
        }
		$this->db->get('ip_quote_custom');
		$this->db->where('quote_id', $quote_id);
		$this->db->set('quote_custom_sent_to', $to . ' ' . date("Y-m-d H:i"));
		$this->db->update('ip_quote_custom');
    }
